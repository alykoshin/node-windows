    ...
    uninstall: {
      ...
      value: function(waitTime){
        ...
        if (!this.exists){
          ...
          //
          // Fix for no event emitted if service is not installed
          //
        ...
        var uninstaller = function(){
          ...
          me.execute('"'+path.resolve(me.root,me._exe)+'" uninstall',function(error,stdout,stderr){
            if (error){
              ...
              //
              // Fix for error on Windows 2008:
              // Error: EPERM: operation not permitted, unlink '<cwd>/daemon/<service-name>.exe'
              //
              var retryTime = 5; // Wait for retryTime if file removal failed
              var rm = function(file){
                if (fs.existsSync(path.join(me.root,file))){
                  try {
                    ...
                    console.log('* Will retry in ' + retryTime + ' seconds');
                    sleep(retryTime);
                    console.log('* Retrying fs.unlinkSync()');
                    try {
                      fs.unlinkSync(path.join(me.root, file));
                    } catch(e) {
