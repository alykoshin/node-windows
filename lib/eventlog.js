...
var write = function(method, log,src,type,msg,id,callback){
  ...
  if (method === 'powershell') {
    msg = msg.replace(/:/g,'');
    msg = msg.replace(/'/g,'');
    wincmd.elevate("powershell.exe -command \"& write-eventlog -LogName "+log+" -Source \"\"\""+src+"\"\"\" -EntryType "+type+" -Category 0 -EventId "+id+" -Message \"\"\""+msg+"\"\"\" \"");
...
var logger = function(config){
  ...
  config.method = config.method || 'eventcreate'; // powershell || eventcreate
  ...
  Object.defineProperties(this,{
    ...
    info: {
      ...
      value: function(message,code,callback) {
        write(config.method, this.eventLog,this.source,'INFORMATION',message,code,callback);
      }
    },
    ...
    error: {
      ...
      configurable: false,
      value: function(message,code,callback) {
        write(config.method, this.eventLog,this.source,'ERROR',message,code,callback);
      }
    },
    ...
    warn: {
      ...
      configurable: false,
      value: function(message,code,callback) {
        write(config.method, this.eventLog,this.source,'WARNING',message,code,callback);
      }
    },
    ...
    auditSuccess: {
      ...
      configurable: false,
      value: function(message,code,callback) {
        write(config.method, this.eventLog,this.source,'SUCCESSAUDIT',message,code,callback);
      }
    },
    ...
    auditFailure: {
      ...
      configurable: false,
      value: function(message,code,callback) {
        write(config.method, this.eventLog,this.source,'FAILUREAUDIT',message,code,callback);
      }
    }
  });
};
